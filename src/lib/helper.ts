...
/**
 * Groupe PlayerSquads by match
 * @param {null| PlayerSquad[]} squads
 * @returns {GroupedSquad}  [string,Match]
 */
export function groupSquadsByMatchId(
  squads: null | PlayerSquad[],
): null | [string, PlayerSquad[]][] {
  if (!squads) return null;

  // Use lodash's groupBy function
  const grouped = _.groupBy(squads, (squad) => squad.matchId);
